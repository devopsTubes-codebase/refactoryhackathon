...
export async function GET(_request: Request, context: { params: { projectId: string } }) {
  try {
    ...
    const docs = await new PostgresDocumentationStore().getDocumentation(context.params.projectId);
    if (!docs) {
      return NextResponse.json({ error: { code: 'NOT_FOUND', message: 'Documentation not found' } }, { status: 404 });
    }

    return NextResponse.json(toDocumentationRetrievalResponse(docs), { status: 200 });
  } catch (error) {
    if (error instanceof UnauthorizedError) {
      return NextResponse.json({ error: { code: 'UNAUTHORIZED', message: 'Authentication is required for this endpoint' } }, { status: 401 });
    }

    const response = toBackendErrorResponse(error, 'Failed to fetch documentation');
