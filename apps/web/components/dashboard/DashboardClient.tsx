...
export function GeneratingDashboard({ progress = generationProgress }: { progress?: GenerationProgress }) {
  return (
    <AppShell>
      <section className="grid min-h-[calc(100vh-64px)] gap-8 px-6 py-20 lg:grid-cols-[0.92fr_1.08fr] lg:px-20">
        <div className="flex flex-col justify-center">
          <h1 className="max-w-[520px] text-[48px] font-bold leading-tight tracking-[-0.05em] text-white">
            Analyzing {progress.projectName}
          </h1>
          <p className="mt-4 max-w-md text-lg leading-7 text-[#a1a1aa]">
            {progress.currentStage}. This presentational state is ready to receive real backend progress updates later.
          </p>
          <div className="mt-12">
              ...
              <div key={stage.id} className="flex gap-4">
                <span
                  className={cn(
                    ...
                    stage.status === 'complete' && 'border-emerald-300 bg-emerald-400/20 text-emerald-200',
                    stage.status === 'active' && 'border-violet-300 bg-violet-400/20 text-violet-200',
                    stage.status === 'pending' && 'border-slate-500 bg-slate-800 text-slate-500'
                  )}
                >
        ...
        <GlassCard className="overflow-hidden bg-black/40">
          <div className="flex h-11 items-center justify-between border-b border-white/10 px-4">
            ...
            <span className="font-mono text-xs text-slate-400">codebase-wiki/analyzer</span>
          </div>
          <div className="h-[620px] overflow-hidden px-6 py-5 font-mono text-sm leading-6 text-[#a5b4fc]/90">
          ...
          <div className="flex items-center justify-between border-t border-white/10 px-6 py-4 text-xs text-slate-400">
            ...
            <span>
              {progress.filesScanned} files · {progress.elapsed} elapsed
            </span>
          </div>
        </GlassCard>
      </section>
